	...
	<div id="speaker2">
		<div id="row">
			<c:forEach items="${organizerList.organizers}" var="organizer"
				...
				<p>
					<a href="#${organizer.firstName}_${organizer.lastName}"><c:out
							value="${organizer.firstName}" /> <c:out
							value="${organizer.lastName}" /></a>
				</p>
			...
			<div id="bio">
				...
				<c:forEach items="${organizerList.organizers}" var="organizer"
					...
					<a style="padding-top: 100px"
						id="${organizer.firstName}_${organizer.lastName}"
						name="${organizer.firstName}_${organizer.lastName}"></a>
					<div class="col-md-4">
						<div id="one-third">
							<c:if test="${organizer.picture != null}">
								<img class="speaker"
									src="${ctx}${baseSiteUrl}/organizers/${organizer.id}.jpg" />
							</c:if>
							...
							<div id="social">
								<ul>
									<c:if test="${!empty organizer.twitterId}">
										<li id="twitter"><a
											href="https://twitter.com/<c:out value="${organizer.twitterId}" />"><img
												class="social"
												alt="<c:out
																							value='${organizer.googlePlusId}' />"
												src="${ctx}/img/icons/icondock/24px/twitter.png" />@<c:out
													value="${organizer.twitterId}" /></a></li>
									</c:if>
