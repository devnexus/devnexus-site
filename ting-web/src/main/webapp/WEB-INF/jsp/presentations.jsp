...
<title>DevNexus 2012 - Presentations</title>
  ...
  <div class="quote"><span>What the community says:</span> "Great conference to share best practices"</div>
    ...
    <ul>
        <c:forEach items="${presentationList.presentations}" var="presentation">
            <li><a href="#id-${presentation.id}"><c:out value="${presentation.title}"/></a></li>
        </c:forEach>
    </ul>

  <c:forEach items="${presentationList.presentations}" var="presentation">
      <div class="presentation">
          <h3 id="id-${presentation.id}" class="title"><c:out value="${presentation.title}"/></h3>
          <c:choose>
            <c:when test="${not empty presentation.speaker}">
                <p class="speaker">
                    <a href="${ctx}${baseSiteUrl}/speakers#${presentation.speaker.firstName}_${presentation.speaker.lastName}">
          ...
          <c:choose>
              <c:when test="${presentation.presentationFile != null}">
                  <p class="download"><a href="${ctx}${baseSiteUrl}/presentations/${presentation.id}/slides">Download Presentation</a></p>
              </c:when>
          </c:choose>
